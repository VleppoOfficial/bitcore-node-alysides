...
function start(options) {
  ...
  if (fullConfig.daemon) {
    start.spawnChildProcess(options.path, process);
  }
 ...
 * located in the path.
 * @param {String} path - The data directory where the bitcoin blockchain and config live.
 * @param {Object} _process - The process that needs to fork a child and then, itself, exit.
 */
function spawnChildProcess(path, _process) {
  ...
  var stderr = fs.openSync(path + '/bitcore-node.log', 'a+');
  var stdout = stderr;
