...
Bitcoin.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['getSaplingWitness', this, this.getSaplingWitness, 2], //CHMEX https://github.com/zerocurrencycoin/bitcore-node-zero/commit/b5c3fc943a14325d902213f4539150d8273a580f
    ['getSaplingBlocks', this, this.getSaplingBlocks, 2],  //CHMEX https://github.com/zerocurrencycoin/bitcore-node-zero/commit/7c08c31c59459af64feeaaeea7bc3fca4c4b6b15
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  log.info('Using alysides config file:', configPath);
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('Alysides Daemon Ready');
          callback();
        });
      });
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Alysides Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Alysides Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned alysides process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean alysides process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting alysides process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Alysides process unexpectedly exited with code:', code);
        log.warn('Restarting alysides child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Alysides process restarted');
          });
...
Bitcoin.prototype.start = function(callback) {
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Alysides configuration options "spawn" or "connect" are expected'));
    }
...
// CHMEX added https://github.com/zerocurrencycoin/bitcore-node-zero/commit/7c08c31c59459af64feeaaeea7bc3fca4c4b6b15
/**
...
//CHMEX added https://github.com/zerocurrencycoin/bitcore-node-zero/commit/b5c3fc943a14325d902213f4539150d8273a580f
/**
